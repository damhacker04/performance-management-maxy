...
class EvaluateDailyTaskJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        $task = DailyTaskEntry::with(['weeklyTarget', 'user'])->find($this->dailyTaskEntryId);
        ...
        // Proses link bukti kerja
        $linkStatus  = 'no_link';
        $linkContent = null;
        $proofUrl    = $task->proof_url;

        if ($proofUrl) {
            $validation = $validator->check($proofUrl);
            $linkStatus = $validation['status'];

            if ($linkStatus === 'public') {
                $linkContent = $extractor->extract($proofUrl);
            }
        }
