            ...
            <div class="m-card" style="padding:0;{{ $isActiveWeek ? 'border:1.5px solid var(--maxy-navy);' : '' }}">
                ...
                <div style="padding:8px 12px 12px;">
                    <a href="{{ route('daily-tasks.create', ['weekly_target_id' => $wt->id]) }}"
                       class="btn btn-block"
                       style="background:transparent;color:var(--maxy-navy);font-size:12px;font-weight:600;
                              border:1.5px dashed var(--maxy-navy);text-decoration:none;
                              display:flex;align-items:center;justify-content:center;gap:4px;padding:9px;">
                        <svg class="lucide sm" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
                        Tambah laporan untuk minggu ini
                    </a>
                </div>
            </div>
        @endforeach
