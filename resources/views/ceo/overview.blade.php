...
<div class="page" style="display:flex;flex-direction:column;gap:20px;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;">
        <div>
            <h1 style="font-size:18px;font-weight:700;color:var(--fg-1);margin:0;">Progress Staf</h1>
            <p style="font-size:13px;color:var(--fg-4);margin:2px 0 0;">Ringkasan kinerja seluruh staf · {{ $monthLabel }}</p>
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
            ...
            <a href="{{ route('monthly-targets.create') }}" class="btn btn-primary" style="white-space:nowrap;">＋ Target untuk Leader</a>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;">
        ...
        @foreach($cards as $c)
            <div class="m-card" style="padding:16px;">
                <div style="font-size:12px;color:var(--fg-4);font-weight:600;text-transform:uppercase;letter-spacing:.04em;">{{ $c['label'] }}</div>
    ...
    <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:20px;align-items:start;">
        ...
        <div class="m-card" style="padding:18px;">
                ...
                <a href="{{ route('period.staff-list', ['year' => $year, 'month' => $month]) }}"
                   style="display:block;text-decoration:none;color:inherit;padding:10px 0;border-bottom:1px solid var(--border,#eee);">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                        ...
                        <span style="font-size:12px;color:var(--fg-4);">{{ $d['staff_count'] }} staf · {{ $d['done'] }}/{{ $d['total'] }} tugas</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px;">
            ...
            @empty
                <p style="font-size:13px;color:var(--fg-4);margin:0;">Belum ada data progress untuk periode ini.</p>
            @endforelse
        </div>

        {{-- Staf perlu perhatian --}}
        <div class="m-card" style="padding:18px;">
            <h2 style="font-size:14px;font-weight:700;color:var(--fg-1);margin:0 0 14px;">⚠ Staf Perlu Perhatian</h2>
                ...
                <a href="{{ route('period.staff-targets', ['year' => $year, 'month' => $month, 'staff' => $r['staff']->id]) }}"
                   style="display:flex;align-items:center;justify-content:space-between;text-decoration:none;color:inherit;padding:8px 0;border-bottom:1px solid var(--border,#eee);">
                    <div style="min-width:0;">
                        <div style="font-size:13px;font-weight:600;color:var(--fg-1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $r['staff']->name }}</div>
                        <div style="font-size:11px;color:var(--fg-4);">{{ \App\Models\User::DEPARTMENTS[$r['staff']->department] ?? ucfirst(str_replace('_',' ',$r['staff']->department ?? '-')) }}</div>
                    </div>
                    <span style="font-size:12px;font-weight:700;color:#DC2626;">{{ $r['progress'] }}%</span>
                </a>
            @empty
                <p style="font-size:13px;color:var(--fg-4);margin:0;">Semua staf dalam kondisi baik 🎉</p>
            @endforelse
        </div>
